...
export default function OdooPurchases() {
  ...
  const load = async (limit = 10, offset = 0, s = stateFilter) => {
    ...
    try {
      ...
      console.error('Error cargando compras Odoo:', err)
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200">
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              {['Referencia', 'Proveedor', 'Fecha', 'Estado', 'Total'].map((h) => (
          ...
          <tbody className="divide-y divide-gray-200">
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-500">Sin pedidos</td></tr>
            ) : items.map((o) => (
