/**
 * Panel de pedidos de compra de Odoo.
 *
 * @author Carlitos6712
 */
import { useEffect, useState } from 'react'
import { listOooPurchases, confirmOooPurchase, cancelOooPurchase } from '@/api/client'
import type { OooPurchase } from '@/types/odoo'
...
export default function OdooPurchases() {
  const [items, setItems] = useState<OooPurchase[]>([])
  ...
  const load = async (limit = 10, offset = 0, s = stateFilter) => {
    ...
    setError(null)
    try {
      const data = await listOooPurchases(limit, offset, s || undefined)
      setItems(data.items)
      setPagination({ limit: data.limit, offset: data.offset, total: data.total, has_more: data.has_more })
    } catch (err) {
      setError((err as Error).message ?? 'Error cargando compras')
    } finally {
      setLoading(false)
    }
  }
  ...
  const runAction = async (id: number, action: 'confirm' | 'cancel') => {
    ...
    setError(null)
    try {
      if (action === 'confirm') await confirmOooPurchase(id)
      else await cancelOooPurchase(id)
